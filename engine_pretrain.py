...
def validate(model, data_loader_val_dict, device, epoch, args, cfg, writer):
    ...
    with torch.no_grad():
        ...
        if 'colorization' in data_loader_val_dict.keys():
            evaluate_colorization.evaluate_colorization_in_training(args, model.module, writer, epoch, data_loader_val_dict['colorization'])
        ...
        if 'semantic_segmentation' in data_loader_val_dict.keys():
