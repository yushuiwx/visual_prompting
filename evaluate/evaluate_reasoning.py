...
def evaluate(args):
    ...
    for idx in trange(len(ds)):
        ...
        if args.output_dir:
            Image.fromarray(np.uint8(original_image)).save(os.path.join(args.output_dir, f'original_{idx}.png'))
            Image.fromarray(np.uint8(generated_result)).save(os.path.join(args.output_dir, f'generated_{idx}.png'))
    ...
    with open(os.path.join(args.output_dir, 'log.txt'), 'a') as log:
        log.write('reasoning all\t'+str(eval_dict)+'\n')
    

def evaluate_reasoning_in_training(args, model, writer, epoch):
    # with open(os.path.join(args.output_dir, 'log.txt'), 'w') as log:
    #     log.write(str(args)+'\n')
    default_task_type = 'size_shape'
    ds = {
        'size': SizeChangeTask,
        'size_color': ChangeSizeColorTask,
        'size_shape': ChangeSizeShapeTask,
    ...
    for idx in range(len(ds)):
        ...
        # Calculate the original_image and the result
        original_image, generated_result = _generate_result_for_canvas(args, model, inpt_pairs)
        original_image = round_image(original_image, ds.color_options() + [BLACK]) #映射到BLACK，Green，White
        generated_result = round_image(generated_result, ds.color_options()+ [BLACK])
        # if args.output_dir:
    ...
    if args.distributed and dist.get_rank() == 0:
        ...
        print('reasoning evaluating: ' + str_out)

        writer.add_scalar('Reasoning-IOU', eval_dict['iou'], global_step=epoch, walltime=None)
        writer.add_scalar('Reasoning-CB-IOU', eval_dict['color_blind_iou'], global_step=epoch, walltime=None)
        writer.add_scalar('Reasoning-ACC', eval_dict['accuracy'], global_step=epoch, walltime=None)
