...
@torch.no_grad()
def generate_image_with_output_logits(orig_image, model, ids_shuffle, len_keep: int, device: str = 'cpu'):
    """ids_shuffle is [bs, 196]"""
    mask, orig_image, x = generate_raw_prediction(device, ids_shuffle, len_keep, model, orig_image)
    L_mask = mask.clone()
    num_patches = 14
...
def decode_raw_predicion(mask, model, num_patches, orig_image, y):
    ...
    orig_image = (
        torch.clip((orig_image[0].cpu().detach()) * 255, 0, 255).int()).unsqueeze(0)
