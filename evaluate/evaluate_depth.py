...
def evaluate(args):
    ...
    ds = DatasetDepth(args.data_path, image_transform, image_transform)
...
def evaluate_depth_in_training(args, model, writer, epoch, ds):

    eval_dict = {'mse': 0.}
    if args.save_validate_image_results:
        sample_freq = max(int(len(ds) // 20), 1)
        save_dir = os.path.join(args.output_dir, 'validate_results_visualization', 'epoch' + str(epoch), 'depth_estimation')
    ...
    if args.distributed and dist.get_rank() == 0:
        ...
        print('depth estimation evaluating: ' + str_out)
        writer.add_scalar('Depth-Estimation-MSE', eval_dict['mse'], global_step=epoch, walltime=None)
