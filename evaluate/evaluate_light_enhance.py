...
from evaluate.light_enhance_dataloader import Datasetlight_enhance
...
def evaluate(args):
    ...
    ds = Datasetlight_enhance(args.data_path, image_transform, image_transform)
...
def evaluate_light_enhance_in_training(args, model, writer, epoch, ds):
    eval_dict = {'mse': 0.}
    if args.save_validate_image_results:
        sample_freq = max(int(len(ds) // 20), 1)
        save_dir = os.path.join(args.output_dir, 'validate_results_visualization', 'epoch' + str(epoch), 'light_enhance')
    ...
    if args.distributed and dist.get_rank() == 0:
        ...
        print('light enhance evaluating: ' + str_out)
        writer.add_scalar('Light-Enhance-MSE', eval_dict['mse'], global_step=epoch, walltime=None)
